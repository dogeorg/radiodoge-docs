<p align="center">
  <img src="public/img/radiodoge-logo.png" alt="RadioDoge" width="280" />
</p>

# RadioDoge Documentation

This repository contains the documentation for [RadioDoge](https://radio.dogecoin.org) — send and receive Dogecoin transactions using radio waves.

The site is built with [Fumadocs](https://www.fumadocs.dev/) (Next.js + MDX), with a flat structure at the repo root (similar to [dogebox-retail](https://github.com/dogeorg/dogebox-retail/)).
...
git clone https://github.com/dogecoinfoundation/radiodoge-docs.git
cd radiodoge-docs
```
...
From the repo root (`radiodoge-docs`):
...
- **Edit or add doc pages:** MDX files in `content/docs/`, e.g. `content/docs/getting-started.mdx`.
...
1. **Fork** the repo on GitHub (click **Fork** on the [radiodoge-docs](https://github.com/dogecoinfoundation/radiodoge-docs) page).
2. **Create a branch** in your fork (e.g. `git checkout -b docs-fix-typo`).
3. **Commit** your changes (e.g. `git add .` then `git commit -m "Fix typo in getting-started"`).
...
| `components/` | React components (layout, home, ai) |
| `content/docs` | MDX docs and meta |
| `lib/` | Source, layout shared options |
| `styles/` | Global and hero CSS |

## GitHub Actions

- **CI** (`.github/workflows/ci.yml`): runs `pnpm install` and `pnpm build` on every push and pull request to `main`/`master`.
- **Deploy** (`.github/workflows/deploy.yml`): on push to `main`/`master`, builds a static export and deploys to GitHub Pages.

To publish via GitHub Pages: go to the repo **Settings → Pages**, set **Source** to **GitHub Actions**. After the next push to `main`, the site will be built and deployed. With a `CNAME` file (e.g. `radio.dogecoin.org`), Pages will serve the site on that domain.

## Legacy static site

The original single-page HTML site is in `old-static/` (css, img, js, index.html). To fully switch to Fumadocs, use the GitHub Actions deploy above and point the `CNAME` / GitHub Pages to the new build.

## Links

- **Live docs (legacy):** [radio.dogecoin.org](https://radio.dogecoin.org)
- **RadioDoge repo:** [github.com/dogecoinfoundation/radiodoge](https://github.com/dogecoinfoundation/radiodoge)
